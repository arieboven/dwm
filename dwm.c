...
static int bh, blw = 0;      /* bar height and symbol width*/
		...
		for (k = start; k <= end; k++)
			for (i = 0; i < LENGTH(keys); i++)
				...
				if (keys[i].keysym == syms[(k - start) * skip])
					for (j = 0; j < LENGTH(modifiers); j++)
						XGrabKey(dpy, k,
							 keys[i].mod == MODMASK ? AnyModifier : keys[i].mod | modifiers[j],
							 root, True,
	...
	if (display != NULL) {
		...
		if (resm != NULL) {
			...
			if (xrdb != NULL) {
				...
				XRDB_LOAD_COLOR("dwm.sellinecolor", sellinecolor);
				XRDB_LOAD_COLOR("dwm.normlinecolor", normlinecolor);
			}
		}
	}
...
{
	if (!selmon->realfullscreen) {
		for(selmon->last_layout = (Layout *)layouts; selmon->last_layout != selmon->lt[selmon->sellt]; selmon->last_layout++);
		setlayout(&((Arg) { .v = &layouts[1] }));
		selmon->oldshowbar = selmon->showbar;
		if (selmon->showbar)
			togglebar(arg);
		selmon->realfullscreen = 1;
	} else {
		selmon->realfullscreen = 0;
...
updatesystray(int updatebar)
{
	...
	if (!systray) {
		...
		wa.border_pixel = 0;
		wa.colormap = cmap;
		systray->win = XCreateWindow(dpy, root, x, -bh, w, bh, 0, depth,
						InputOutput, visual,
						CWOverrideRedirect|CWBackPixel|CWBorderPixel|CWColormap|CWEventMask, &wa);
	...
	for (w = 0, i = systray->icons; i; i = i->next) {
		...
		w += systraypadding;
		i->x = w;
		XMoveResizeWindow(dpy, i->win, i->x, bhpadding / 2, i->w, i->h);
		w += i->w;
		if (i->mon != m)
			i->mon = m;
	}
	w = w ? w + systraypadding : 1;
	XMoveResizeWindow(dpy, systray->win, x, (w == 1 || !m->showbar) ? -bh : m->by, w, bh);
	wc.x = x; wc.y = (w == 1 || !m->showbar) ? -bh : m->by;
	wc.width = w; wc.height = bh;
...
zoom(const Arg *arg)
{
	...
	if (!selmon->lt[selmon->sellt]->arrange || !c || c->rules & IsFloating)
		return;
